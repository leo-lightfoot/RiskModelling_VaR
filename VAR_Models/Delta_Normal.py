...
# Use absolute paths
portfolio_nav_path = "C:/Users/abdul/Desktop/Github Repos/RiskModelling_VaR/portfolio_results/portfolio_nav_history.csv"
portfolio_returns_path = "C:/Users/abdul/Desktop/Github Repos/RiskModelling_VaR/portfolio_results/portfolio_returns_history.csv"

# Load NAV data
df_NAV = pd.read_csv(portfolio_nav_path)
...
df_portfolio_returns = pd.read_csv(portfolio_returns_path)
...
# Create a copy with Date as index for VaR calculation
...
# -----------------------------------------------------------------------
#   METHOD 2: Treating Individual Securities with Normality Assumption
# -----------------------------------------------------------------------
# Compute log returns from NAVs (used in covariance method)
...
# Validate calculated returns vs reported returns
...
# Improved covariance VaR with dynamic weights
def compute_var_covariance_matrix(nav_returns, window, nav_df):
    ...
    for i in range(window, len(nav_returns)):
        # Use dynamic weights based on the most recent NAV values
...
# Calculate VAR with dynamic weights
...
# For full period, use most recent weights
...
# Calculate t-distribution VaR for comparison (to handle fat tails)
def compute_var_t_distribution(returns, window, nav_df):
    rolling_var = []
    rolling_dates = []
    
    # Use fixed degrees of freedom of 5
    degrees_freedom = 5
    ...
    for i in range(window, len(returns)):
        ...
        # T-distribution quantile with fixed degrees of freedom = 5
        t_quantile = t.ppf(1 - confidence_level, degrees_freedom)
        
        # Calculate VaR with appropriate scaling factor for t-distribution
        scaling_factor = np.sqrt((degrees_freedom - 2) / degrees_freedom)  # For df=5: sqrt(3/5)
...
# Calculate t-distribution VaR for 250-day window
...
# Basic plotting without error handling or saving files
...
# Simple diagnostics for backtesting
